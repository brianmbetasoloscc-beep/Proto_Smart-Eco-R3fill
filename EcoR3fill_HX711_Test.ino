...
const int LOADCELL_DOUT_PIN = A0; // Data pin (DO) to Gizduino A0
const int LOADCELL_SCK_PIN = A1;  // Clock pin (SCK) to Gizduino A1
...
void setup() {
  ...
  // Check if HX711 is communicating
  if (scale.is_ready()) {
    Serial.println("HX711 is ready!");
  } else {
    Serial.println("--- ERROR: HX711 not found. Check wiring. ---");
    while (true); // Halt if HX711 is not found
  }
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (command == 't') { // Tare command
      Serial.println("\nTaring... please wait.");
      scale.tare();
      Serial.println("Scale tared to 0.");
      Serial.println("Place a known weight on the scale and enter its value (e.g., '100g') and press 'c'.");
    } else if (command == 'c') { // Calculate factor command
      // Read the known weight from Serial input
      String weightStr = Serial.readStringUntil('g'); // Read until 'g' character
      ...
      // Read a few raw values and average them
      long raw_reading = scale.get_value(10); // Get average of 10 readings
      ...
      if (raw_reading != 0) {
        ...
        Serial.println(new_calibration_factor, 2); // Print with 2 decimal places
        Serial.println("-------------------------");
  ...
  // Display current raw reading (useful for monitoring stability)
  if (scale.is_ready()) {
